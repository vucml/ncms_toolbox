functions {
    real cmr_lpmf(int[] recall_sequence, int LL, real init_fc, real init_cf, real beta_enc, real beta_rec, real beta_start, real P1, real P2, real stopInit, real stopScale, real stopShape, real gamma) {
    ...
    int[] cmr_rng(int LL, real init_fc, real init_cf, real beta_enc, real beta_rec, real beta_start, real P1, real P2, real stopInit, real stopScale, real stopShape, real gamma) {
        int num_context_units = LL + 1;
        int num_item_units = LL;
        int start_index = LL + 1;
...
parameters {
    ...
    // 2 real<lower=0> init_cf;
    // 3 real<lower=0, upper=1> beta_enc;
    // 4 real<lower=0, upper=1> beta_rec;
    // 5 real<lower=0, upper=1> beta_start;
    // 6 real<lower=0> P1;
    // 7 real<lower=0> P2;
    // 8 real<lower=0, upper=1> stopInit;
    // 9 real<lower=0> stopScale;
    // 10 real<lower=0> stopShape;
    // 11 real<lower=0> gamma;
}
transformed parameters {
...

    {
        ...
        subjPars[2] = exp(parMean[2] + subjParsR[2]);
        subjPars[3] = inv_logit(parMean[3] + subjParsR[3]);
        subjPars[4] = inv_logit(parMean[4] + subjParsR[4]);
        subjPars[5] = inv_logit(parMean[5] + subjParsR[5]);
        subjPars[6] = exp(parMean[6] + subjParsR[6]);
        subjPars[7] = exp(parMean[7] + subjParsR[7]);
        subjPars[8] = inv_logit(parMean[8] + subjParsR[8]);
        subjPars[9] = exp(parMean[9] + subjParsR[9]);
        subjPars[10] = exp(parMean[10] + subjParsR[10]);
        subjPars[11] = exp(parMean[11] + subjParsR[11]);
    }
}
model {
    parMean ~ normal(0, 5);
    ...
    subjSD ~ exponential(0.2);

    for (i in 1:nSeq) {
        recall_sequences[i] ~ cmr(LL[i], subjPars[1, subj[i]], subjPars[2, subj[i]], subjPars[3, subj[i]], subjPars[4, subj[i]], subjPars[5, subj[i]], subjPars[6, subj[i]], subjPars[7, subj[i]], subjPars[8, subj[i]], subjPars[9, subj[i]], subjPars[10, subj[i]], subjPars[11, subj[i]]);
    }
}
generated quantities {
    ...
    for (i in 1:nSeq) {
        pred_recall[i] = cmr_rng(LL[i], subjPars[1, subj[i]], subjPars[2, subj[i]], subjPars[3, subj[i]], subjPars[4, subj[i]], subjPars[5, subj[i]], subjPars[6, subj[i]], subjPars[7, subj[i]], subjPars[8, subj[i]], subjPars[9, subj[i]], subjPars[10, subj[i]], subjPars[11, subj[i]]);
    }
}
