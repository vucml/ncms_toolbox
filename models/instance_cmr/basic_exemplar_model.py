...
# Here we attempt to reproduce and test a version of the MINERVA 2 exemplar
# model as originated by Hintzman (1984, 1986, 1988) and extended by Jamieson et al (2018) to model semantic memory.
#
# ## Overview
...
# %% [markdown]
# ## Experiment
# We reproduce the information in Figure 2 from Jamieson et al (2018) to confirm that the
...
#
# A simple artificial language is constructed to generate a corpus experiences (Table 1 of paper), consisting of 12
# words sorted between 7 lexical categories and 3 sentence frames specifying how trios of words from different
# categories can be associated within a verbal context.
#
...
# 4. We retrieve an echo for each of the individual words.
# 5. We compute the similarity between the echo retrieved for each word against the echo retrieved for each of the
#       other 11 words.
# 6. We'll also retrieve the echo for an ambiguous word (`break`) in conjunction with other words and
#       compare the resulting echoes with the echoes for unambiguous words to confirm that the model can achieve word
#       disambiguation.
#
# ### Experiences
...
similarities = []
for probe_A in tqdm(word_list):
    similarities.append([])
    for probe_B in word_list:
        similarities[-1].append(model.compare(word_vectors[probe_A], word_vectors[probe_B]))
        print(probe_A, probe_B, similarities[-1][-1])
